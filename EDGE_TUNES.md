...
| TUNE-10 | Earned-loosening exit redesign | exit | shadow-active | V7 pre-registered; results pending after-close authoritative --score + multiple-testing correction + OOS-forward N>=30 (NOT the partial preview); +vol-state-adaptive initial stop + time-stop + OR-invalidation stop as harness variants; +alpha-decay-by-horizon diagnostic (match stop/exit to the 15-60min ORB hold) | gating: live-exit instrumentation (now passing) | PROP-EXIT-FALSE-STOPOUT | V7 | 2026-06-19 |
...
| TUNE-12 | Stop-distance tuning (0.15 vs 1.0 ATR) | exit | shadow-active | V1 wide-init in harness; results pending OOS-forward N>=30 + multiple-testing correction (1-day preview NOT a verdict); +vol-state-adaptive stop variant | gating: V0 gate (now passing) | PROP-EXIT-FALSE-STOPOUT | V1_WIDE | 2026-06-19 |
...
| TUNE-14 | Re-entry discipline (cap vs fresh-breakout gate) | reentry | shadow-active | R0-R3 logged, in-sample inconclusive (all perm-p insignificant, N=2) | - | PROP-REENTRY-DRAG | reentry | 2026-06-19 |
