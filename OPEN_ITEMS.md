...
- **V8 shadow test (TUNE-08b, two-adverse-closes exit)** -- Rhett green-lit; SHADOW only, NOT live. Runs daily (7th EOD action, v8_shadow_log.jsonl). Initial read: flat (-$36) + VIOLATES must-not-cut (4 winners->losers). Accumulating OOS over the coming days; promote only on OOS+N>=30+0-cut+net-positive, else kill.
