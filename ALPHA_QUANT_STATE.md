...
### 2026-06-12 — Re-entry / Exit / 2 investigations (from SMCI daily-review handoff)
- **Re-entry:** no explicit "already-traded-today" exclusion exists. Re-entry was blocked only because multi-scan was OFF; the multi-scan re-arm (`orb_multiscan.py:94`) already excludes ONLY held/working names ("one active position per name"), so a flat re-qualifying symbol already re-arms through the identical edge gate. Multi-scan ON (6/11, live 6/12) delivers it. **Remaining = Nth-occurrence tagging (DERIVED post-hoc from broker truth, no bot change) + per-Nth post-cost expectancy measurement.** SIM, default-allow; filter only if re-entries underperform 1st-entries after cost at N≥30 + gauntlet.
- **Exit redesign:** QUEUED for A/B, NOT default. Pushback: a real intra-bar 0.15×ATR **stop order already exists** (proven — SMCI's 29.62 working SELL stop, UROUT'd at trail engage). New work = +1R partial scale-out + profit-adaptive ATR trail + drop candle-close + keep a real stop live through the trail phase (the actual violent-drop gap). A/B behind a flag vs candle-close, same entries, replay+forward SIM, N≥30, costs subtracted, **segmented green/red/flat**, then `falsification_gauntlet`. Never SIM-default until it wins.
- **Investigation 1 (SMCI mechanism): RESOLVED.** 9:35-armed breakout **DAY stop** (opened 9:36 ET, filled 12:48 ET, 663 @ 30.14) that rested ~3h. NOT multi-scan (`ORB_MULTISCAN` turned ON 5:26 PM 6/11, after close — was OFF all day). Narrative invented windows from fill hour; `trade_analytics` scan_window **FIXED** to attribute 0935. New open question: ORB entries are DAY orders with **no intraday entry cutoff** (stale morning level can fire hours late) — decision pending.
- **Investigation 2 (MFE bug): FIXED.** MFE from 1-min bar highs fell below realized when the exit filled above the max bar high (SMCI +391 < +411). Floored favorable excursion by the exit fill → MFE ≥ realized. Re-reviewed SMCI: MFE 411.06 = realized, left-in-trade 0; the real leak was **$795.60 AFTER exit** (post-exit continuation, day high 32.05). Cached 6/11 narratives queued for regeneration.
