...
> **Generated:** 2026-09-23 16:51:15 Eastern Daylight Time · **coordination-repo HEAD:** `a749bcf` · by `strategy-research/system_facts.py` (read-only).
...
| SAFE_MODE_ENFORCE (gate teeth) | False | `tradestation-bot/risk_config.py:227` |
| consecutive_clean streak | 6 | `validation/clean_day_certifier.py:consecutive_clean()` |
...
| Universe generated_at | 2026-09-23T07:30:30.923442-04:00 | `advisor_universe_latest.json:generated_at` |
...
| In-play gate (RelVol/move/$-vol) | False  (OFF) | `tradestation-bot/risk_config.py:302` |
| HTB/halted exclusion | True  (ON) | `tradestation-bot/risk_config.py:346` |
...
| Deploy-controller scope | True -- governs the RE-ARM/multiscan admit ceiling only; 9:35 ORB sizes by its own constants | `tradestation-bot/risk_config.py:277` |
| Deploy base | $400,000 | `tradestation-bot/risk_config.py:276` |
| Deploy target % | 0.95 | `tradestation-bot/risk_config.py:280` |
| Deploy target $ (computed live) | $380,000 | `deploy_controller.deploy_target() = DEPLOY_BASE * DEPLOY_TARGET_PCT` |
| Per-position notional cap | $100,000 | `tradestation-bot/risk_config.py:285` |
| Per-side cap | 0.50 ($200,000/side) | `tradestation-bot/risk_config.py:286` |
| Max open positions (count backstop) | 4 | `tradestation-bot/risk_config.py:94` |

## EXIT
| Fact | Live value | Source |
|---|---|---|
| Resting broker stop distance | min(1.4 x ATR, $2500/qty)  (StopMarket; from-entry $-CAP ACTIVE, Loop 187 -- caps single-position loss at $2500 from fill, broker-resting/stream-independent) | `tradestation-bot/orb_runner.py:101` |
...
| Unconfirmed candle exit (LIVE 2026-09-20) | before confirmation, exit on the first opposite-direction 1-min close whose BODY is >= 0.15 x ATR. Small opposite candles are IGNORED (median 1-min body is 0.0158 ATR). Both long and short paths wired in exit_bot_v2. Reason EXIT_UNCONFIRMED_REVERSAL. Reversible via UNCONFIRMED_CANDLE_EXIT_ENABLED. | `tradestation-bot/candle_close_exit.py:83` |
| Unconfirmed tightening floor (LIVE 2026-09-20) | an UNCONFIRMED position's stop floor shrinks with age: after 10 min -> 0.50 x the dollar-stop floor then after 20 min -> 0.25 x the dollar-stop floor. Confirmed positions keep the full floor, and any missing/bad trade age falls back to the FULL floor (fail-safe). Reason EXIT_UNCONFIRMED_TIGHTENED. | `tradestation-bot/candle_close_exit.py:85` |
| Unconfirmed time-stop (Loop 220, LIVE 2026-07-01) | 30 min -> flatten if STILL UNCONFIRMED (favorable excursion never crossed +0.15xATR); confirmed positions UNTOUCHED. Fires in exit_bot_v2 for every open position incl. TW-owned (before the lease); reason TIME_EXIT_<N>M_UNCONFIRMED (does not count toward the 2-stops/day breaker). SIM-only, reversible via UNCONFIRMED_TIME_STOP_ENABLED. | `tradestation-bot/risk_config.py:168` |
...
| Entries with a broker resting stop | 1192/1369 = 87.1% | `strategy-research/stop_coverage_audit.py (re-derived from broker_orders_unified.csv)` |
| 9:35 cohort coverage | 591/596 = 99% | `stop_coverage_audit.py` |
| Re-arm/late cohort coverage | 601/773 = 78% | `stop_coverage_audit.py` |
| Median placement latency | 258s (~4.3 min) | `stop_coverage_audit.py` |
| PRIMARY protection mechanism | software poll (exit_bot_v2 + chandelier)  (mechanisms: {'resting_stop_HIT': 51, 'candle/chandelier': 1247, 'eod_flatten': 71}) | `stop_coverage_audit.py exit-mechanism tally` |
